...
void CPU::Draw(uint8_t x, uint8_t y, const uint8_t height) {
...
uint8_t CPU::GetDelay() { return mDelay_timer; }

void CPU::SetDelay(uint8_t value) { mDelay_timer = value; }

void CPU::SetSoundTimer(uint8_t value) { mSound_timer = value; }

uint16_t CPU::GetSpriteAddr(uint8_t character) {
  return FONTS_OFFSET + (5 * character);
}

/* Store V0 to VX (r - included) in memory, starting at address I */
void CPU::DumpRegisters(int r_index, uint16_t addr) {
...
void CPU::Execute() {
  ...
  case 0xD000: // Draw a sprite at (VX, VY), that has a width of 8 pixels and
    ...
    Draw(mV[(mOpcode & 0x0F00) >> 8], mV[(mOpcode & 0x00F0) >> 4],
         mOpcode & 0x000F);
    break;
