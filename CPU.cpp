...
void CPU::clearScreen() { memset(screen, 0, SCREEN_H * SCREEN_W); }
...
uint8_t CPU::getDelay() { return delay_timer; }
...
void CPU::setDelay(uint8_t value) { delay_timer = value; }
    ...
    if ((opcode & 0xF000) >= 0x3000 && (opcode & 0xF000) <= 0x9000) {
      ...
      case 0x9000: // Refactor for 0x5XY0, 0x8XYN, 0x9XYN
      {
        ...
        case 0x8003: // Set VX = (VX ^ VY)
          V[x] = (V[x] ^ V[y]);
          break;
        case 0x8004: // Set VX += VY, VF is set to 1 when there's a carry, and
          ...
          V[x] += V[y];
          break;
        case 0x8005: // Set VX -= VY, VF is set to 0 when there's a borrow, and
                     // 1 when there isn't
        ...
        case 0x8006: // Shift Vx = Vy >> 1
          ...
          V[x] = V[y] >> 1;
          break;
        case 0x8007: // Set VX = VY - VX, VF is set to 0 when there's a borrow,
